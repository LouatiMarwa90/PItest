...
public class AjouterEvenement {
    ...
    void Ajouter(ActionEvent event) {
        ...
        if (nom_id.getText().isEmpty() || Lieux_id.getText().isEmpty() || nbr_max_id.getText().isEmpty()) {
        ...
        prodData.setType(Type.valueOf(nom_id.getText()));
        prodData.setLieux(Lieux_id.getText());
        prodData.setNbMax(Integer.parseInt(nbr_max_id.getText()));
        prodData.setImage_eve(imagePath); // Ajouter le chemin de l'image
        ...
        try {
            ...
            Date dateD = Date.from(dated_eve_local.atStartOfDay(ZoneId.systemDefault()).toInstant());
            Date dateF = Date.from(datef_eve_local.atStartOfDay(ZoneId.systemDefault()).toInstant());

            prodData.setDateD(dateD);
            prodData.setDateF(dateF);
            ...
            showErrorMessage("Une erreur inattendue s'est produite : " + e.getMessage());
            e.printStackTrace(); // Ajoutez cette ligne pour plus de détails sur l'erreur
        }
    }
...
//    }
    ...
    public void SelectData() {
        ...
        if (prodData != null) {
            nom_id.setText(String.valueOf(prodData.getType()));
            Lieux_id.setText(prodData.getLieux());

            // Convertir java.util.Date en java.sql.Date
            java.sql.Date dated_eve_sql = new java.sql.Date(prodData.getDateD().getTime());
            java.sql.Date datef_eve_sql = new java.sql.Date(prodData.getDateF().getTime());
            ...
            nbr_max_id.setText(String.valueOf(prodData.getNbMax()));
...
//  }
    ...
    public void delete () {
        ...
        try {
            int id_eve = prodData.getIdEv(); // Supposons que getId_eve() renvoie l'identifiant de l'événement
